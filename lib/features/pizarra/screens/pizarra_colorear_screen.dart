// lib/features/pizarra/screens/pizarra_colorear_screen.dart
...
import 'pizarra_colorear_lista_screen.dart';

class PizarraColorearScreen extends StatefulWidget {
  const PizarraColorearScreen({super.key});

  @override
  State<PizarraColorearScreen> createState() => _PizarraColorearScreenState();
}
...
class _DrawingPoint {
  ...
  _DrawingPoint({required this.point, required this.paint});
}

class _PizarraColorearScreenState extends State<PizarraColorearScreen> {
  final List<_DrawingPoint?> _puntos = [];
  Color _colorActual = Colors.red;
  final double _grosorActual = 9.0;
  bool _modoBorrador = false;
  bool _mostrarPaleta = false;
  bool _inicializado = false;
  late ColorearItem _item;
  ...
  void didChangeDependencies() {
    ...
    _item = args is ColorearItem ? args : dibujosParaColorear.first;
    _inicializado = true;
  }
  ...
  Paint _crearPaint() {
    return Paint()
      ...
      ..strokeWidth = _modoBorrador ? 32.0 : _grosorActual
  ...
  void _agregarPunto(Offset punto) {
    setState(() {
      ...
      _puntos.add(_DrawingPoint(point: punto, paint: _crearPaint()));
    });
  }
    ...
    final confirmar = await showDialog<bool>(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: const Text(
            'Se limpiará todo lo que pintaste.',
            textAlign: TextAlign.center,
            style: TextStyle(
        ...
        child: Stack(
          ...
          children: [
            ...
            GestureDetector(
              ...
              child: CustomPaint(
                painter: _ColorearPainter(_puntos),
...
class _ColorearPainter extends CustomPainter {
  final List<_DrawingPoint?> points;

  _ColorearPainter(this.points);
